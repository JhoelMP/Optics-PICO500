...
while ~isempty(ray_index)
    ...
    diffuse_cut = surfacetype_next==1 | surfacetype_next==2;
    n_diffuse = sum(surfacetype_next==1);
    n_retro = sum(surfacetype_next==2);
    %n_diffuse = sum(diffuse_cut);
    cos_theta = ones(n_diffuse+n_retro,1);
    sin_theta = zeros(n_diffuse+n_retro,1);
    theta = zeros(n_diffuse,1);
    rng('shuffle');
    
    short_surf = surfacetype_next;
    short_surf(short_surf==0)=[];
    
    cos_theta(short_surf==1) = rand(n_diffuse,1);
    theta(short_surf==2) = normrnd(0,deg2rad(1),[n_retro,1]);
    
    cos_theta(short_surf==2) = cos(theta(short_surf==2));
    cos_theta(cos_theta > 1) = 2 - cos_theta(cos_theta > 1);
    cos_theta(cos_theta < 0) = zeros(size(cos_theta(cos_theta < 0)));
    sin_theta = sqrt(1-cos_theta.^2);
            
    phi = rand(n_diffuse+n_retro,1) * 2 * pi;
    
    x_tmp = cross(s_next(diffuse_cut,:), repmat([1 0 0],n_diffuse+n_retro,1), 2);
    y_tmp = cross(s_next(diffuse_cut,:), repmat([0 1 0],n_diffuse+n_retro,1), 2);
    ...
    % rays coming in at a shallow angle to retroreflector surface may be
    % reflected into the surface based on the Gaussian distribution about
    % the incidence angle
    ...
    stdR = deg2rad(5); %standard deviation
    angR = acos(sum(sm_next .* s_next,2)); % angle between incident ray and surface normal
    refl_I = exp(-angR.^2/(2*stdR^2)); % Gaussian for reflected intensity
    ...
    temp_rays = reflected_rays(:,7:10);
    temp_I = repmat(refl_I,1,4);
    temp_rays(surfacetype_next == 2) = temp_rays(surfacetype_next == 2) .* temp_I(surfacetype_next == 2);
    reflected_rays(:,7:10) = temp_rays;
